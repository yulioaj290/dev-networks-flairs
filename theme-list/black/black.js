(function(themes) {
    themes.githubBlackFlair = `
    <!-- --------- Loaded from theme [Begin] --------- -->
        <div class="title-container">
            <h1 class="title" data-property="fullname"></h1>
            <h4 class="subtitle">
                @<a href="basic-theme.html#" data-property="username" data-link="url"></a>
            </h4>
        </div>

        <div class="field-container">
            <div class="fields">
                <p class="data">
                    <svg height="12" viewBox="0 0 16 16" width="12">
                        <path fill-rule="evenodd" d="M16 12.999c0 .439-.45 1-1 1H7.995c-.539 0-.994-.447-.995-.999H1c-.54 0-1-.561-1-1 0-2.634 3-4 3-4s.229-.409 0-1c-.841-.621-1.058-.59-1-3 .058-2.419 1.367-3 2.5-3s2.442.58 2.5 3c.058 2.41-.159 2.379-1 3-.229.59 0 1 0 1s1.549.711 2.42 2.088C9.196 9.369 10 8.999 10 8.999s.229-.409 0-1c-.841-.62-1.058-.59-1-3 .058-2.419 1.367-3 2.5-3s2.437.581 2.495 3c.059 2.41-.158 2.38-1 3-.229.59 0 1 0 1s3.005 1.366 3.005 4"></path>
                    </svg>
                    <span data-property="followers"></span>
                    <svg height="12" viewBox="0 0 12 16" width="12">
                        <path fill-rule="evenodd" d="M4 9H3V8h1v1zm0-3H3v1h1V6zm0-2H3v1h1V4zm0-2H3v1h1V2zm8-1v12c0 .55-.45 1-1 1H6v2l-1.5-1.5L3 16v-2H1c-.55 0-1-.45-1-1V1c0-.55.45-1 1-1h10c.55 0 1 .45 1 1zm-1 10H1v2h2v-1h3v1h5v-2zm0-10H2v9h9V1z"></path>
                    </svg>
                    <span data-property="repos"></span>
                    <svg height="12" viewBox="0 0 12 16" width="12">
                        <path fill-rule="evenodd" d="M7.5 5L10 7.5 7.5 10l-.75-.75L8.5 7.5 6.75 5.75 7.5 5zm-3 0L2 7.5 4.5 10l.75-.75L3.5 7.5l1.75-1.75L4.5 5zM0 13V2c0-.55.45-1 1-1h10c.55 0 1 .45 1 1v11c0 .55-.45 1-1 1H1c-.55 0-1-.45-1-1zm1 0h10V2H1v11z"></path>
                    </svg>
                    <span data-property="gists"></span>
                </p>
                <p class="data">
                    <svg height="12" viewBox="0 0 12 16" width="12">
                        <path fill-rule="evenodd" d="M6 0C2.69 0 0 2.5 0 5.5 0 10.02 6 16 6 16s6-5.98 6-10.5C12 2.5 9.31 0 6 0zm0 14.55C4.14 12.52 1 8.44 1 5.5 1 3.02 3.25 1 6 1c1.34 0 2.61.48 3.56 1.36.92.86 1.44 1.97 1.44 3.14 0 2.94-3.14 7.02-5 9.05zM8 5.5c0 1.11-.89 2-2 2-1.11 0-2-.89-2-2 0-1.11.89-2 2-2 1.11 0 2 .89 2 2z"></path>
                    </svg>
                    <span data-property="location"></span>
                </p>
                <p class="data">
                    <a href="basic-theme.html#" data-property="website" data-link="website">Blog / Website</a>
                </p>
                <p class="data">
                    <span data-property="company"></span>
                </p>
            </div>
            <div class="image">
                <img src="#" alt="" data-property="avatar" data-link="src">
            </div>
        </div>

        <div class="corner">
            <svg width="40" height="40" viewBox="0 0 250 250" aria-hidden="true">
                <path d="M0,0 L115,115 L130,115 L142,142 L250,250 L250,0 Z"></path>
                <path d="M128.3,109.0 C113.8,99.7 119.0,89.6 119.0,89.6 C122.0,82.7 120.5,78.6 120.5,78.6 C119.2,72.0 123.4,76.3 123.4,76.3 C127.3,80.9 125.5,87.3 125.5,87.3 C122.9,97.6 130.6,101.9 134.4,103.2"
                    \ fill="currentColor" style="transform-origin: 130px 106px;" class="octo-arm"></path>
                <path d="M115.0,115.0 C114.9,115.1 118.7,116.5 119.8,115.4 L133.7,101.6 C136.9,99.2 139.9,98.4 142.2,98.6 C133.8,88.0 127.5,74.4 143.8,58.0 C148.5,53.4 154.0,51.2 159.7,51.0 C160.3,49.4 163.2,43.6 171.4,40.1 C171.4,40.1 176.1,42.5 178.8,56.2 C183.1,58.6 187.2,61.8 190.9,65.4 C194.5,69.0 197.7,73.2 200.1,77.6 C213.8,80.2 216.3,84.9 216.3,84.9 C212.7,93.1 206.9,96.0 205.4,96.6 C205.1,102.4 203.0,107.8 198.3,112.5 C181.9,128.9 168.3,122.5 157.7,114.1 C157.9,116.9 156.7,120.9 152.7,124.9 L141.0,136.5 C139.8,137.7 141.6,141.9 141.8,141.8 Z"
                    \ fill="currentColor" class="octo-body"></path>
            </svg>
        </div>
    <!-- --------- Loaded from theme [End] --------- -->
    `;

    themes.stackofBlackFlair = `
    <!-- --------- Loaded from theme [Begin] --------- -->
        <div class="title-container">
            <h1 class="title" data-property="fullname" data-link="url"></h1>
            <h4 class="subtitle">
                <span data-property="reputation"></span>
            </h4>
        </div>

        <div class="field-container">
            <div class="fields">
                <p class="data">
                    <span class="badge-gold" data-property="badge-gold"></span>
                    <span class="badge-silver" data-property="badge-silver"></span>
                    <span class="badge-bronze" data-property="badge-bronze"></span>
                </p>
                <p class="data">
                    <span data-property="questions"></span>
                    Questions
                    &nbsp;&nbsp;&nbsp;
                    <span data-property="answers"></span>
                    Answers
                </p>
                <p class="data">
                    <svg height="12" viewBox="0 0 12 16" width="12">
                        <path fill-rule="evenodd" d="M6 0C2.69 0 0 2.5 0 5.5 0 10.02 6 16 6 16s6-5.98 6-10.5C12 2.5 9.31 0 6 0zm0 14.55C4.14 12.52 1 8.44 1 5.5 1 3.02 3.25 1 6 1c1.34 0 2.61.48 3.56 1.36.92.86 1.44 1.97 1.44 3.14 0 2.94-3.14 7.02-5 9.05zM8 5.5c0 1.11-.89 2-2 2-1.11 0-2-.89-2-2 0-1.11.89-2 2-2 1.11 0 2 .89 2 2z"></path>
                    </svg>
                    <span data-property="location"></span>
                </p>
                <p class="data">
                    <a href="basic-theme.html#" data-property="website" data-link="website">Blog /
                        Website</a>
                </p>
            </div>
            <div class="image">
                <img src="#" alt="" data-property="avatar" data-link="src">
            </div>
        </div>

        <div class="corner">
            <svg width="40" height="40" viewBox="0 0 200 200">
                <path class="st-back" d="M0,200 L200,200 L200,0 Z"></path>
                <path class="st-box" d="M159.4 173.8V150.6h7.7v30.9H97.6V150.6h7.7v23.2z" />
                <path class="st-bars" d="M113.8 148.4l37.8 7.9 1.6-7.6-37.8-7.9-1.6 7.6zm5-18l35 16.3 3.2-7-35-16.4-3.2 7.1zm9.7-17.2l29.7 24.7 4.9-5.9-29.7-24.7-4.9 5.9zm19.2-18.3l-6.2 4.6 23 31 6.2-4.6-23-31zM113 166h38.6v-7.7H113V86z" />
            </svg>
        </div>
    <!-- --------- Loaded from theme [End] --------- -->
    `;
})((window.flairThemes = {}));
